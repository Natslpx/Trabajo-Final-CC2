...
struct Player {
    ...
    int rawHorizontal() const {
        if (isInputDown(LEFT)) return -1;
        if (isInputDown(RIGHT)) return 1;
        return 0;
    }

    int rawVertical() const {
        if (isInputDown(UP)) return -1;
        if (isInputDown(DOWN)) return 1;
        return 0;
    }
    ...
    void draw() const {       
        ...
        if (isGrabbing) {
            if (rawVertical()!=0) {
                ...
                spriteRect = facing ? Rectangle{(timer/10) * width, 0, width, height} : Rectangle{((timer/10) + 1) * width, 0, -width, height};
            } else {
                climbing_timer = 0;
            ...
            spriteRect =  facing ? Rectangle{0, 0, width, height} : Rectangle{width, 0, -width, height};
        }
        bool isTired = stamina < 30;
        DrawTextureRec(sprite, spriteRect, position - Vector2{width/2, height}, isTired ? RED : WHITE);
    }
    ...
    void update(std::vector<std::unique_ptr<EnvItem>> &envItems, float delta) {
        ...
        if (rawHorizontal() == 1.0f) facing = true;
        else if (rawHorizontal() == -1.0f) facing = false;
        ...
        if (isInputPressed(JUMP) && canJump) { //Salto
            ...
            hasJumped = true;
            onGround = false;
        } else if (isInputPressed(JUMP) && !isDashing && (onWallLeft || onWallRight)) { //Wall Jump
            velocity.y = -jumpSpeed;
        ...
        if (!onGround) { //Movimiento vertical aéreo
            if ((onWallLeft || onWallRight) && (isInputDown(GRAB1) || isInputDown(GRAB2)) && !isWallJumping && stamina>0) { //Climb
                isGrabbing = true;
                if (rawVertical()!=0) {
            ...
            } else if (!isInputDown(JUMP) && velocity.y < 0 && hasJumped) { //Salto corto
    ...
    Player() :  sprite_base(LoadTexture("../images/monkey_base.png")), 
                ...
                isInputDown([](int KEY) {return IsKeyDown(KEY);}), 
                isInputPressed([](int KEY) {return IsKeyPressed(KEY);}),
                UP((int)KEY_UP), DOWN((int)KEY_DOWN), LEFT((int)KEY_LEFT), RIGHT((int)KEY_RIGHT), 
                JUMP((int)KEY_C), DASH((int)KEY_X), GRAB1((int)KEY_Z), GRAB2((int)KEY_V)
    {} //Jugador principal (teclado)

    Player(int gamepadNumber) :  sprite_base(LoadTexture("../images/monkey_base.png")), 
                sprite_base_dashed(LoadTexture("../images/monkey_dashed.png")), 
                sprite_grabing(LoadTexture("../images/monkey_grabbing.png")), 
                sprite_grabbing_dashed(LoadTexture("../images/monkey_grabbing_dashed.png")),
                sprite_walking(LoadTexture("../images/monkey_walking.png")),
                ...
                sprite_jumping(LoadTexture("../images/monkey_jumping.png")),
                sprite_jumping_dashed(LoadTexture("../images/monkey_jumping_dashed.png")),
                isInputDown([gamepadNumber](int BUTTON) {return IsGamepadButtonDown(gamepadNumber, BUTTON);}), 
                isInputPressed([gamepadNumber](int BUTTON) {return IsGamepadButtonPressed(gamepadNumber, BUTTON);}),
                UP((int)GAMEPAD_BUTTON_LEFT_FACE_UP), DOWN((int)GAMEPAD_BUTTON_LEFT_FACE_DOWN), LEFT((int)GAMEPAD_BUTTON_LEFT_FACE_LEFT), RIGHT((int)GAMEPAD_BUTTON_LEFT_FACE_RIGHT), 
                JUMP((int)GAMEPAD_BUTTON_RIGHT_FACE_DOWN), DASH((int)GAMEPAD_BUTTON_RIGHT_FACE_LEFT), GRAB1((int)GAMEPAD_BUTTON_LEFT_TRIGGER_1), GRAB2((int)GAMEPAD_BUTTON_RIGHT_TRIGGER_1)
    {} //Jugador de mando
    ...
    Player(Player&& other) noexcept : 
        sprite_base(other.sprite_base), sprite_base_dashed(other.sprite_base_dashed),
        sprite_grabing(other.sprite_grabing), sprite_grabbing_dashed(other.sprite_grabbing_dashed),
        sprite_walking(other.sprite_walking),
        sprite_climbing(other.sprite_climbing), sprite_climbing_dashed(other.sprite_climbing_dashed),
        sprite_jumping(other.sprite_jumping), sprite_jumping_dashed(other.sprite_jumping_dashed),
        UP(other.UP), DOWN(other.DOWN), LEFT(other.LEFT), RIGHT(other.RIGHT),
        JUMP(other.JUMP), DASH(other.DASH), GRAB1(other.GRAB1), GRAB2(other.GRAB2),
        isInputDown(std::move(other.isInputDown)), isInputPressed(std::move(other.isInputPressed)) {
...
struct Text : EnvItem {
    ...
    const int y;
    const int fontSize;
    Text(std::string _text, int _x=0, int _y=0, int _fontSize = 100, Color _color = BLACK) : text(_text), x(_x), y(_y), fontSize(_fontSize), EnvItem(_color) {}
